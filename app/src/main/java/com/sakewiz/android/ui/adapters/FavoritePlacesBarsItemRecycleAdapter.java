 ...
 */
public class FavoritePlacesBarsItemRecycleAdapter extends RecyclerView.Adapter<FavoritePlacesBarsItemRecycleAdapter.FavoritePlacesBarsItemRowHolder>  {

    private List<Place> favoritePlacesBarItemList;
    private Activity mContext;
    private FavoritePlacesBarsFragment favoritePlacesBarsFragment;
    private int lastPosition = -1;


    public FavoritePlacesBarsItemRecycleAdapter(Activity context, FavoritePlacesBarsFragment favoritePlacesBarsFragment, List<Place> favoritePlacesBarsItemList) {
        this.favoritePlacesBarItemList = favoritePlacesBarsItemList;
        this.favoritePlacesBarsFragment = favoritePlacesBarsFragment;
        this.mContext = context;
    }

    @Override
    public FavoritePlacesBarsItemRowHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View v = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.favorite_sake_item, null);
        FavoritePlacesBarsItemRowHolder mh = new FavoritePlacesBarsItemRowHolder(v);
    ...
    @Override
    public void onBindViewHolder(final FavoritePlacesBarsItemRowHolder favoritePlacesBarsItemRowHolder, final int position) {
        final Place item = favoritePlacesBarItemList.get(position);
        if(item != null) {
            if (item.getImgUrl() != null && !item.getImgUrl().isEmpty()) {
                createImageView(item.getImgUrl(), favoritePlacesBarsItemRowHolder.productImage, favoritePlacesBarsItemRowHolder.imageProgress);
            } else {
                favoritePlacesBarsItemRowHolder.imageProgress.setVisibility(View.GONE);
            }
            if (item.getName() != null)
                favoritePlacesBarsItemRowHolder.mSakeTitle.setText(item.getName().getEn());
            if (item.getType() != null && !item.getType().isEmpty())
                favoritePlacesBarsItemRowHolder.mSakeType.setText(item.getType());
...
//            }
            ...
            if (item.getRate() != null && !item.getRate().isEmpty()) {
                ...
                switch (rate) {
                    case "1.0":
                        favoritePlacesBarsItemRowHolder.starPoint_1.setImageResource(R.drawable.ic_star);
                        break;
                    case "2.0":
                        favoritePlacesBarsItemRowHolder.starPoint_1.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_2.setImageResource(R.drawable.ic_star);
                        break;
                    case "3.0":
                        favoritePlacesBarsItemRowHolder.starPoint_1.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_2.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_3.setImageResource(R.drawable.ic_star);
                        break;
                    case "4.0":
                        favoritePlacesBarsItemRowHolder.starPoint_1.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_2.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_3.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_4.setImageResource(R.drawable.ic_star);
                        break;
                    case "5.0":
                        favoritePlacesBarsItemRowHolder.starPoint_1.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_2.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_3.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_4.setImageResource(R.drawable.ic_star);
                        favoritePlacesBarsItemRowHolder.starPoint_5.setImageResource(R.drawable.ic_star);
                        break;
                    default:
                        break;
                }
            ...
            if (item.getReviewCount() != null && !item.getReviewCount().isEmpty())
                favoritePlacesBarsItemRowHolder.mCommentCount.setText(item.getReviewCount());
            if (item.getFavoured() != null && !item.getFavoured().isEmpty())
                favoritePlacesBarsItemRowHolder.mLikeCount.setText(item.getFavoured());
            if (item.getUpdated() != null && !item.getUpdated().isEmpty())
                favoritePlacesBarsItemRowHolder.mTime.setText(TimeUtils.getRelativeTime(Long.parseLong(item.getUpdated()))
...
//                    FavoritePlacesBarsFragment.favoritePlacesBarsFragment.createMarker(item.getLat(), item.getLon());
        ...
        setAnimation(favoritePlacesBarsItemRowHolder.parentContainer, position);

        favoritePlacesBarsItemRowHolder.parentContainer.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        favoritePlacesBarsItemRowHolder.deleteButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                favoritePlacesBarsFragment.performUnFavourPlaceBarRequest(item.getId());
            }
        });
    ...
    @Override
    public void onViewDetachedFromWindow(FavoritePlacesBarsItemRowHolder holder) {
        super.onViewDetachedFromWindow(holder);
        ((FavoritePlacesBarsItemRowHolder) holder).clearAnimation();
    }

    public class FavoritePlacesBarsItemRowHolder extends RecyclerView.ViewHolder {
        ...
        public FavoritePlacesBarsItemRowHolder(View view) {
            super(view);
